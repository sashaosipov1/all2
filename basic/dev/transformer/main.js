(function () {
    let cont = document.querySelector('#container_a2_transformer');
    ...
    function changeCSS(cssFile, cssLinkIndex) {
        let oldlink = doc.getElementsByTagName("link").item(cssLinkIndex);
    ...
    var getPlayerMarkup = function () {
        ...
        fr.style.justifyContent = 'center';
        fr.style.zIndex = 9999;
        fr.style.overflow = 'hidden';

        // Стартовое определение формата контейнера
        targetPosition = {
            top: window.scrollY + cont.getBoundingClientRect().top,
            bottom: window.scrollY + cont.getBoundingClientRect().bottom
        };
    ...
    function initPlayer(vastStr) {
        ...
        if (vpaid) {
            ...
            if (!mobileBool) {
                ...
                fr.style.top = document.documentElement.clientHeight - fr.height - 15 + 'px';
                fr.style.left = document.documentElement.clientWidth - contWidth - 15 + 'px';
            } else {
                fr.width = '80%';
                fr.height = v[0].offsetHeight + 40;
                fr.style.top = document.documentElement.clientHeight - fr.height + 'px';
                fr.style.left = '0px';
            }
            ...
            js.onload = (e) => {
                ...
                const environmentVars = {
                    slot: document.querySelector('#container_a2_transformer'),
    ...
    function PlayingParams() {
        ...
        targetPosition = {
            top: window.scrollY + cont.getBoundingClientRect().top,
            bottom: window.scrollY + cont.getBoundingClientRect().bottom + parseInt(fr.height)
        };
        console.log(targetPosition);
    }
